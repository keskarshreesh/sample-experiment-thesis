...
from networks import decoder, bg_append_conv
from config import opts
import random
import numpy as np

def get_reconstructed_image(tgt_pose, src_app, tgt_bg, reuse=False):
    # concatenate src pose and app embedding
    tgt_pose_src_app_embedding = tf.concat([tgt_pose, src_app], 3)             # Bx16x16x(256+14)
    tgt_pose_src_app_activation = decoder(tgt_pose_src_app_embedding, reuse=reuse)
    print ("tgt_pose_src_app_embedding ", tgt_pose_src_app_embedding)
    tgt_bg_black = tf.zeros_like(tgt_bg)
    tgt_pose_src_app_image = bg_append_conv(tgt_pose_src_app_activation, tgt_bg, reuse=reuse)                # Bx224x224x3
    tgt_pose_src_app_vis = bg_append_conv(tgt_pose_src_app_activation, tgt_bg_black, reuse=True)       # Bx224x224x3
    print ("tgt_pose_src_app_image ", tgt_pose_src_app_image)
    return tgt_pose_src_app_image, tgt_pose_src_app_vis
...
def gaussian_heatmaps(points):
    ...
    gauss_xy = []
    gauss_xy1 = []
    map_sizes = [opts['image_size']//16, opts['image_size']]
    print ("points ", points)
    ...
    print ("points_exchanged ", points_exchanged)
    ...
    print ("gauss_xy ", gauss_xy)
    print ("gauss_xy1 ", gauss_xy1)
...
def get_limb_centers(joints_2d):
    ...
    for i in range(1, joints_2d.shape[1]):
        ...
        # print ("angles ", angles)
    print ("angles_x ", angles_x)
    print ("angles_y ", angles_y)
    ...
    print ("limbs_x ", limbs_x)
    print ("limbs_y ", limbs_y)
...
def limb_maps(pose_points):
    ...
    print ("limb_centers_yx ", limb_centers_yx)
    ...
    map_sizes = [opts['image_size']//16, opts['image_size']//8, opts['image_size']//4]
    gauss_xy = []
    for map_size in map_sizes:
